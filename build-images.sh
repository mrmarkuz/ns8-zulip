...
# Reuse existing nodebuilder-zulip container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-zulip; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-zulip -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
# --label="org.nethserver.images=docker.io/mariadb:10.11.5 docker.io/roundcube/roundcubemail:1.6.12-apache"
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/postgres:17.10-alpine docker.io/nginx:1.30.0-alpine" \
    "${container}"
# Commit the image
